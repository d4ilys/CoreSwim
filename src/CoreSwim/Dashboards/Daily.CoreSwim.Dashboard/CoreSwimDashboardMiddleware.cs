...
{
    internal class CoreSwimDashboardMiddleware
    {
        private readonly StaticFileMiddleware _staticFileMiddleware;
        private readonly CoreSwimDashboardOptions _options;

        public CoreSwimDashboardMiddleware(RequestDelegate next,
            IWebHostEnvironment hostingEnv,
            ILoggerFactory loggerFactory,
            CoreSwimDashboardOptions options)
        {
            _options = options;
        ...
        StaticFileMiddleware CreateStaticFileMiddleware(
            ...
            CoreSwimDashboardOptions options)
        {
            string embeddedFileNamespace = "Daily.CoreSwim.Dashboard";
            var staticFileOptions = new StaticFileOptions
            {
                RequestPath = string.IsNullOrEmpty(options.DashboardPath) ? string.Empty : $"/{options.DashboardPath}",
                FileProvider = new EmbeddedFileProvider(
                    typeof(CoreSwimDashboardMiddleware).GetTypeInfo().Assembly,
        ...
        private async Task RespondWithIndexHtmlAsync(HttpResponse response)
        {
            ...
            await using var stream = typeof(CoreSwimDashboardExtension).GetTypeInfo().Assembly
                .GetManifestResourceStream("Daily.CoreSwim.Dashboard.index.html");
